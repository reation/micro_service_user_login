...
type (
	userModel interface {
		...
		FindInfoByUserName(ctx context.Context, userName string) (*User, error)
...
func (m *defaultUserModel) FindInfoByUserName(ctx context.Context, userName string) (*User, error) {
	var resp User
	query := fmt.Sprintf("select %s from %s where `user_name` = ? limit 1", userRows, m.table)
	err := sqlx.SqlConn.QueryRow(ctx, &resp, query, userName)
	if err != nil {
		return &resp, err
	}
...
func (m *defaultUserModel) Insert(ctx context.Context, data *User) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.UserName, data.Password, data.Nickname, data.Sex, data.Birthday, data.State, data.IsMember)
	}, userIdKey)
	return ret, err
}
...
func (m *defaultUserModel) Update(ctx context.Context, data *User) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.UserName, data.Password, data.Nickname, data.Sex, data.Birthday, data.State, data.IsMember, data.Id)
	}, userIdKey)
	return err
}
